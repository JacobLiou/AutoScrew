...
| 1.6 | 2026-08-17 | 拧紧参数/顺序页导出工艺卡 TXT 与顺序 Excel（设备调教→模板闭环） |
...
| — | **工艺工作台 / 工艺库** | 参数·顺序·来源 + Excel 导入 + **导出 TXT/Excel 模板**（2026-08-17） |
...
5 现在针对拧紧参数的模板提出了疑问，采用TXT编辑但是要兼顾所有情形 比如现在针对拧紧参数的高阶设定有一些参数也要设置 这里的模板应该配置成什么样最好呢  业务方只认定TXT 其实json应该是最好的形式 


请注意 最后可能都没有MES的概念 现在业务方不打算对接MES 但是会通过GetProductKey来获取PN SN绑定关系 同时把PN SN所有数据都上传局域网 包括工艺卡 比如拧紧参数 拧紧顺序以及作业台数据
