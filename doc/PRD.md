...
2. 操作员按产品图与顺序**锁附**（电批扳机或半自动触发；开工 `#303` 激活工艺库顺序）
3. 实时曲线采集与显示；**OK/NG 以设备周期结果为准**（上位机不另用 Recipe 阈值锁 NG）
...
单钉作业编排（V1.4）:
  - 供料: 操作员现场手动（上位机不调度）
  - 换参: 作业开始 `#303` 激活工艺库拧紧顺序，设备按步骤（参数 × 数量）步进；作业台不再每钉 `#302`
  - 锁附: 电批拧紧周期 + 扭矩–角度曲线采集
  - 判定: **产线以 IEMD-SD 周期 IsOk 为准**（浮锁/滑牙/斜锁/卡钉/过扭由设备拧紧参数保障）；上位机采集曲线用于显示与归档，`LockCurveEvaluator` 仅 advisory 日志，不锁屏；漏锁仍由上位机校验
...
- 换产下发拧紧参数（`#100`）与拧紧顺序（`#200`）。
- 作业开始：`#300` 保持手动切换 + `#303` 激活该 PN 的主顺序（SequenceId 最小）；**不写** `#301` 来源绑定表。
- 每钉：**不发 `#302`**。设备按顺序步骤（参数 × 数量）步进。
- 上位机仍负责 SN/MES、模板引导、曲线归档；模板/`local-recipes` 的 `ControllerParameterId` 不参与换参。

拧紧来源页 / HostGuided 模式下拉仍保留供 FAT，作业台不再读取该模式。

**边界**：不替代 EHS/急停/互锁；二期视觉导航若引入需另开 PRD 条目。

**FAT 建议**：工艺库换产 → 扫 SN → 连续多钉确认设备参数按顺序步进、不随钉号跳参。
